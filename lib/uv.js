...
async function uvToRequirements() {
    ...
    try {
        ...
        // Add the extra index URL to the requirements.txt (howso platform)
        const original = await readFile(requirementsPath, 'utf8');
        await writeFile(requirementsPath, "--extra-index-url 'https://mySecretPypiToken@pypi.howso.becausal.com/simple/'" + '\n' + original);
